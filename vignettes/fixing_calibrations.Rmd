...
```{r}
# We should be able to use this as is, to combine the sensor data with the 
# annotated calibration data
...
sensor_calibration_data <- cal_join_sensor_calibration_data(
  ...
  calibration_data_list = annotated_cal_data
) 
```
...
```{r save-cal-data}
write_rds(calibrated_data, here("..", "poudre_sonde_network", "data", "manual_data_verification", "complete_dataset", "calibrated_sensor_data.rds"))
```
...
# Create a combined dataset for plotting - exclude uncalibrated data
plot_data <- bind_rows(
  # Original data
  calibrated_test %>% 
    select(DT_round, value = mean) %>%
    mutate(sensor_serial = "Original", cal_status = "Reference"),
  
  # Only calibrated sensor data
  calibrated_test %>%
    filter(cal_check == TRUE) %>%  # Only keep calibrated data
    select(DT_round, value = mean_lm_trans, sensor_serial, cal_check) %>%
    mutate(cal_status = "Calibrated")
)

# Define distinct colors for the sensors
sensor_colors <- c("1048153" = "tomato",  
                   "1095007" = "steelblue",  
                   "859922" = "darkgreen",    
                   "Original" = "lightgrey")   
...
calibration_p_test <- ggplot(plot_data) +
  ...
  scale_linetype_manual(values = c("Reference" = "solid", 
                                   "Calibrated" = "dashed"),
                        name = "Status") +
  labs(
    title = paste(site, parameter, "2024 Calibration"),
...
I am highlighting sensors to be able to recognize sensor swaps for this draft. 

Here we see some data that is not corrected. Presumably the first instance of this
happened when sensor `1095007` was swapped out for sensor `859922` and was not 
calibrated again that year. There seems to have been another sensor swap for sensor
`859922` later in the year, but this is not tracked in the data. I checked for 
weirdness in the data processing steps, but there was nothing in those steps outputs 
code that tipped me off of a potential bug, though I am definitely not ruling
that out. 

Now that we are using sensor specific calibration provenance for back calibration, 
I would expect for these kinds of situations to happen more frequently, especially
for data near the end of a year. If a sensor was swapped out and not calibrated 
again for the year, this is what we would expect from the data.
