...
vignette: >
  %\VignetteIndexEntry{Fixing Drift}
...
sensor_f <- list.files(here::here("data", "raw", "sensor", "manual_data_verification",
                       ...
                       pattern = "Turbidity|FDOM Fluorescence", 
                       ignore.case = TRUE,
                       full.names = TRUE)

data <- map(sensor_f, arrow::read_parquet) %>%
  set_names(stringr::str_remove(basename(sensor_f), "_FINAL.*$"))
...
```{r}
plot_drift(data$`archery_virridy-FDOM Fluorescence`)
```
...
drift_correction_chunks <- drift_chunks %>% 
  keep(~ any(.x$drift == "TRUE") & !all(is.na(.x$mean_analysis)))

# Make a subset of the data that does not need to be corrected
no_correction_chunks <- drift_chunks %>% 
  keep(~ any(.x$drift == "FALSE") | all(is.na(.x$mean_analysis)))
```
...
```{r}
good_pre_drift_col <- mean(tail(drift_correction_chunks$`2`$mean_analysis, 4))

# Filtering out NA data to get to the numeric data for the average result
good_drift_post_val_df <- no_correction_chunks$`5` %>%
  filter(!is.na(mean_analysis))

good_post_drift_col <- mean(head(good_drift_post_val_df$mean_analysis, 4))
...
# Adding in additional data for context
min_date <- min(good_example$DT_round, na.rm = TRUE)
max_date <- max(good_example$DT_round, na.rm = TRUE)

#Get pre/post data for plotting
extra_data <- data$`riverbluffs-Turbidity` %>%
  filter(
    (DT_round >= (min_date - days(2)) & DT_round < min_date) | 
    (DT_round > max_date & DT_round <= (max_date + days(2)))
  )

nearby_data <- data$`archery-Turbidity` %>%
  filter(
    (DT_round >= (min_date - days(1)) & DT_round <= (max_date + days(1)))
  )

# Plots to compare the results
# Good example plots ====
p_good_lin_add <- ggplot(good_lin_add, aes(x = DT_round)) +
  geom_line(aes(y = mean_analysis, color = "Uncorrected"), linewidth = 1) +
  geom_line(aes(y = mean_drift_trans, color = "Corrected"), linewidth = 1) +
    geom_line(data = extra_data, aes(y = mean_analysis, color = "Pre/Post"), linewidth = 1) +
  geom_line(data = nearby_data, aes(y = mean_analysis, color = "Nearby Site"), linewidth = 1, alpha = 0.5) +
  scale_color_manual(values = c("Uncorrected" = "red", "Corrected" = "green", "Pre/Post" = "black", "Nearby Site" = "grey")) +
  labs(title = "Linear + Additive", y = "Turbidity (NTU)", x = "Date", color = NULL) +
  theme_minimal() +
  theme(legend.position = "bottom")
...
min_date <- min(bad_example$DT_round, na.rm = TRUE)
max_date <- max(bad_example$DT_round, na.rm = TRUE)
