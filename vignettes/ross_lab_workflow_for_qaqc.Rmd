---
title: "ROSS Lab QAQC Workflow (PWQN / HydroVu)"
author: "Radical Open Science Syndicate"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{ROSS Lab QAQC Workflow (PWQN / HydroVu)}
...
This vignette documents the production QAQC workflow used by the Radical Open
Science Syndicate (ROSS) lab for the Poudre Watershed Quality Network (PWQN).
It is specific to In-Situ Inc. AquaTroll multiparameter sondes uploading data
to the HydroVu cloud API, with field notes managed through mWater.

For a sensor-agnostic introduction to the package, see
`vignette("using_the_package_in_rstudio")`.
...
# TODO: add back calibration step
# TODO: add drift correction step
```
...
```{r}
site_order_list <- load_site_order("inst/site_order_templates/template_site_orders.yaml")
#Or from CSV
